...
module LeftistTreeMake(Ord: OrderedType) =
  struct
    ...
    let is_empty = BinaryTree.is_empty
    let map f = BinaryTree.map (fun (svalue, x) -> (svalue, f x))
    let _forget tree = BinaryTree.map snd tree
...
module PairingHeapMake(Ord: OrderedType) =
  struct
    ...
    let is_empty = MultiwayTree.is_empty
...
module BinomialHeapMake (Ord : OrderedType) =
  struct
    ...
    let to_arbitrary_seq l =
      l |> List.map MultiwayTree.to_arbitrary_seq |> List.to_seq |> Seq.concat
