...
let drop n s = let rec aux = function
                 | (0, lazy tl) -> tl
                 | (_, lazy Nil) -> Nil
                 | (n, lazy (Cons(_, tl))) -> aux (pred n, tl) in
               lazy(aux(n, s))

let reverse s = let rec aux = function
                  | (lazy Nil, r) -> r
                  | (lazy (Cons(hd, tl)), r) -> aux(tl, Cons(hd, lazy r)) in
                lazy(aux(s, Nil))
let empty = lazy Nil
