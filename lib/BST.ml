...
module DefaultBST(Ord: OrderedType) = struct
  ...
  let rec mem elt = function
    | Empty -> false
    | Node (v, l, r) -> let cmp = Ord.compare elt v in
  ...
  let rec bounds x = 
    ...
    function
    | Empty -> None, None
    | Node(v, l, r) -> let cmp = Ord.compare x v in
                       if cmp = 0 then (Some v, Some v)
                       else if cmp < 0 then let (a, b) = bounds x l in (a, or_else b v)
                       else let (a, b) = bounds x r in (or_else a v, b)
...
module RBTree(Ord: OrderedType) = struct
  ...
  let rec map f = function
    | Empty -> Empty
    | Node (c, v, h, l, r) -> Node(c, f v, h, map f l, map f r)
  ...
  let is_constant seq =
    let open Seq in
    let rec const_equal v s = match s () with
      | Nil -> true
      | Cons (x, xs) -> (x = v) && const_equal v xs in
    match seq () with
    | Nil -> true
    | Cons (x, xs) -> const_equal x xs
       
  let black_vals =
    let rec aux n = function
      | Empty -> Seq.return n
      | Node(c, v, _, l, r) ->
         let n = if c = Black then succ n else n in Seq.append (aux n l) (aux n r) in
    aux 0

  let reds_are_separate =
    let rec aux prev t = match (prev, t) with
    | _, Empty -> true
    | Red, Node(Red, _, _, _, _) -> false
    | Black, Node(c, _, _, l, r) -> (aux c l) && (aux c r) in
    aux Black

  let is_balanced t =
    is_constant @@ black_vals t && reds_are_separate t
  ...
  let rec mem elt = function
    | Empty -> false
    | Node (_, _, v, l, r) -> let cmp = Ord.compare elt v in
                        (cmp = 0) || (cmp < 0 && mem elt l) || (cmp > 0 && mem elt r)

  let rec push elt = function
    | Empty -> leaf elt
    | Node (c, h, v, l, r) -> let cmp = Ord.compare elt v in
                        if cmp <= 0 then Node(c, h, v, push elt l, r) else Node(c, h, v, l, push elt r)
