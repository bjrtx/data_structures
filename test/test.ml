...
module Tester(Q : Heap.PriorityQueue with type elt = int) = struct
  let print_int_list l = List.iter (fun x -> print_int x; print_string " ") l; print_newline ()
  ...
  let () = if qsort <> sorted then (let () = (print_int_list qsort; print_int_list sorted) in assert false) 
end
